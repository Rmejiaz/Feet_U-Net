...
def get_model(output_channels=1, size=224,name="SegNet", kernel = 3, ActFunction = 'selu',dropout = 0, trainable = True):
    ...
    Conv26 = Conv2D(filters=output_channels, kernel_size=(kernel,kernel), padding='same', activation=ActFunction, data_format='channels_last', name='Conv26')(Act25)
    Norm26 = BatchNormalization(name='Norm26')(Conv26)

    Out = Activation('sigmoid', name='OutputLayer')(Norm26)
    return tf.keras.Model(inputs = Input, outputs = Out, name = "Segnet")
